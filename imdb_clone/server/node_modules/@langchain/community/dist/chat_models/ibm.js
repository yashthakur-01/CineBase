...
import { WatsonxToolsOutputParser, _convertToolCallIdToMistralCompatible, authenticateAndSetGatewayInstance, authenticateAndSetInstance, checkValidProps, expectOneOf } from "../utils/ibm.js";
import { BaseChatModel } from "@langchain/core/language_models/chat_models";
import { AIMessage, AIMessageChunk, ChatMessage, ChatMessageChunk, FunctionMessageChunk, HumanMessageChunk, ToolMessageChunk, isAIMessage } from "@langchain/core/messages";
...
var ibm_exports = {};
__export(ibm_exports, { ChatWatsonx: () => ChatWatsonx });
function _convertToValidToolId(model, tool_call_id) {
	if (model.startsWith("mistralai") && tool_call_id) return _convertToolCallIdToMistralCompatible(tool_call_id);
	return tool_call_id;
}
function _convertToolToWatsonxTool(tools) {
	return tools.map((tool) => {
		if ("type" in tool) return tool;
		const parameters = isInteropZodSchema(tool.schema) ? toJsonSchema(tool.schema) : tool.schema;
		return {
			type: "function",
			function: {
				name: tool.name,
...
function _watsonxResponseToChatMessage(choice, rawDataId, usage) {
	...
	switch (message.role) {
		case "assistant": {
			...
			const additional_kwargs = { tool_calls: rawToolCalls.map((toolCall) => ({
				...toolCall,
				type: "function"
			})) };
...
function _convertDeltaToMessageChunk(helperIndex, delta, rawData, model, usage, defaultRole) {
	...
	switch (role) {
		case "user": return new HumanMessageChunk({ content });
		case "assistant": {
		...
		case "tool":
			if (rawToolCalls) return new ToolMessageChunk({
				content,
				additional_kwargs,
				tool_call_id: _convertToValidToolId(model ?? "", rawToolCalls[0].id)
			});
			return null;
		case "function": return new FunctionMessageChunk({
			content,
			additional_kwargs
		});
		default: return new ChatMessageChunk({
			content,
			role
		});
	}
...
var ChatWatsonx = class extends BaseChatModel {
	static lc_name() {
		return "ChatWatsonx";
	}
	...
	async *_streamResponseChunks(messages, options, _runManager) {
		...
		for await (const chunk of stream) {
			...
			const generationChunk$1 = new ChatGenerationChunk({
				message,
				text: delta.content ?? "",
				generationInfo
			});
...
//#endregion
export { ChatWatsonx, ibm_exports };
